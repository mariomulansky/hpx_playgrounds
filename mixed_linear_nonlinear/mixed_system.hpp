...
//
#ifndef MIXED_SYSTEM_HPP
#define MIXED_SYSTEM_HPP
...
shared_vec osc_op1( shared_vec q , shared_vec q_r , shared_vec dpdt )
{
    ...
    // nonlinear coupling to the right
    (*dpdt)[M-1] = -(*q)[M-1] + coupling
...
// center part of the chain (nonlinear)
...
shared_vec osc_op3( shared_vec q , shared_vec q_l , shared_vec dpdt )
{
    ...
    double coupling = pow( abs((*q_l)[M-1] - (*q)[0]) , LAMBDA-1 ) * sign((*q_l)[M-1]-(*q)[0]);
...
void mixed_system( const state_type &q , state_type &dpdt )
{
               ...
               const dvec &p1 , const dvec &p2 , const dvec &p3 )
{
    ...
    {
        e += 0.5*pow<2>( p1[n] ) + 0.5*pow<2>( q1[n] )
            + 0.5*pow<2>( q1[n] - q1[n+1] );
    }
    // nl coupling
    e += 0.5*pow<2>( p1[N1-1] ) + 0.5*pow<2>( q1[N1-1] )
        + pow( abs(q1[N1-1]-q2[0]) , LAMBDA) / LAMBDA;
    ...
    {
        e += 0.5*pow<2>( p3[n] ) + 0.5*pow<2>( q3[n] )
            + 0.5*pow<2>( q3[n] - q3[n+1] );
    }
    e += 0.5*pow<2>( p3[N3-1] ) + 0.5*pow<2>( q3[N3-1] );
